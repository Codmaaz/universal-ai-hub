...
class MediaRepository {
  ...
  Future<List<MediaItem>> list({MediaKind? kind}) async { final db=await AppDatabase.instance; final rows=await db.query('media_items',columns:const ['id','kind','provider_id','provider_name','model','prompt','negative_prompt','source_file_path','remote_url','local_path','status','job_id','created_at','updated_at'],where:kind==null?null:'kind=?',whereArgs:kind==null?null:[kind.name],columns:const ['id','kind','provider_id','provider_name','model','prompt','negative_prompt','source_file_path','remote_url','local_path','status','job_id','created_at','updated_at'],orderBy:'created_at DESC'); return rows.map(MediaItem.fromRow).toList(); }
