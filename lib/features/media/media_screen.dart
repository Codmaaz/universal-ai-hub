...
class _MediaScreenState extends State<MediaScreen> {
  ...
  Future<void> _go() async {
    ...
    try {
      if (_image) {
        await _service.generateImage(providerId: _providerId!, model: _model.text.trim().isEmpty ? 'auto' : _model.text.trim(),
          prompt: _prompt.text.trim(), size: _imageSize, n: _count, negativePrompt: _negative.text.trim());
      } else {
        await _service.startVideo(providerId: _providerId!, model: _model.text.trim().isEmpty ? 'auto' : _model.text.trim(),
  ...
  String _friendlyError(Object e) { final s = e.toString(); return s.replaceFirst('Exception: ', '').replaceFirst("Instance of 'ResponseBody'", 'Provider returned an unreadable error response'); }

  @override Widget build(BuildContext context) {
    final providers = AppScope.of(context).state.providers.where((p) => p.isEnabled).toList();
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        DropdownButtonFormField<String>(value: _providerId, decoration: const InputDecoration(labelText: 'Provider'),
          items: providers.map((p) => DropdownMenuItem(value: p.id, child: Text(p.name))).toList(), onChanged: (v) => setState(() => _providerId = v)),
  ...
  Widget _card(MediaItem item) {
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (url != null && _image) ...[const SizedBox(height: 8), Image.network(url, errorBuilder: (_, __, ___) => const Text('Image preview unavailable'))],
      ...
      const SizedBox(height: 4), Wrap(spacing: 4, children: [
        ...
        if (url != null) OutlinedButton.icon(onPressed: _busy ? null : () => _download(item), icon: const Icon(Icons.download), label: const Text('Download')),
